...
describe("conservative institution resolution", () => {
  ...
  it.each([
    ...
    ["parenthesized-at footer email", `<footer>Example University · Example City, Example Country · info(at)example.edu</footer>`],
    ["bracketed-at footer email", `<footer>Example University · Example City, Example Country · info [at] example.edu</footer>`],
  ...
  it("rejects an obfuscated contact address outside the official domain", async () => {
    const resolve = createResolver({ lookup: async () => [identity], search: async () => [],
      fetchPage: async () => publisherFixture(`<title>Example University</title><h1>Example University</h1>
        <footer>Example City, Example Country · info(at)unrelated.example</footer>`) });
