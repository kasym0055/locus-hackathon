...
- [ ] Install pinned `@upstash/redis` and the `redis` client as a development-only dependency for local/CI Lua tests. Use a local Redis test service when available; otherwise use an isolated namespace on the authorized free store for the initial task, then CI uses its own local service. Configure separate development, release/live-acceptance and judging pools from the actual available Brave credit. When 1,000 calls are actually available, allocate development a maximum of 600 calls, release/live acceptance a maximum of 200 calls, and protect a judging reserve of at least 200 calls. These are ceilings, not spending targets; development must not consume the acceptance allocation, and neither phase may consume the judging reserve. If actual available credit differs, protect the judging reserve first, then scale down or cut development and acceptance ceilings so their combined allowance plus the reserve never exceeds the verified balance. If fewer than 200 calls are available, protect the remaining calls for judging and report the shortfall; a larger balance does not automatically raise the default development or acceptance ceilings. No automatic paid overage. Record no keys in Git.
...
- [ ] Before any live run, verify remaining credits and the remaining release/live-acceptance pool established in Task 3. With 1,000 initially available Brave calls, development has a maximum of 600, release/live acceptance has a maximum of 200, and at least 200 remain reserved for judging. The acceptance ceiling covers all acceptance invocations, cold runs, additional scenarios, concurrency checks, retries and rechecks together; it is not an additional 200-call allowance per run. These ceilings are not spending targets. Set the command's caps no higher than the remaining phase allowance and authorized credit/cash allowance; stop when either is reached. If actual available credit is lower, scale down or cut development and acceptance usage while protecting the judging reserve; never borrow from that reserve to complete the matrix. Twenty cold runs at ten calls each could exhaust the entire acceptance pool, so additional scenarios must fit through lower actual call usage or be reported as incomplete. If the allowance cannot cover the approved matrix, report the incomplete gate rather than purchasing or claiming completion silently.
...
- Brave allocation consistency rechecked: when 1,000 calls are actually available, the default allocation is 600 development + 200 release/live acceptance + 200 protected judging calls = 1,000. Development and acceptance values are ceilings, not spending targets; a larger judging reserve requires correspondingly lower nonjudging allowances. Task 11 draws from the single acceptance pool, including retries/rechecks and additional scenarios; it cannot consume the judging reserve. Lower balances require reduced nonjudging ceilings after protecting the reserve, and higher balances do not automatically increase those ceilings. No task interfaces or other requirements change.
